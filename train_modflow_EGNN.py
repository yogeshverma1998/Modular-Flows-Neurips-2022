...
diffeq = GraphFlow_EGNN(len(Unique_elements),len(Unique_elements))
    ...
    if itr%1 == 0 or itr == args.niters:
        with torch.no_grad():
            ...
            if total_test_loss < best_loss:
                ...
                torch.save(model,str(cwd) + "/Models/" + "modflow_2d_"+str(args.data)+"_model_" + str(itr) + ".pt")
                
            model.train()


logger.info('Training is finished and models are saved in the respective directories.')
