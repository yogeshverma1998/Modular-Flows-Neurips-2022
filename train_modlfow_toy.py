...
final_data = []
#loading molecular dat
cwd = os.getcwd()
samples_to_consider = args.nsamples
batch_size = args.nsamples
for i in range(batch_size):
    if args.data == "stripes":
        data = alternate_strip_data_grid()
    elif args.data == "4x4_chess" :
        data = alternate_checker_data_grid_mod()
    else:
        data =  alternate_checker_data_grid()
    ...
    if itr%1 == 0 or itr == args.niters:
        with torch.no_grad():
            ...
            if total_test_loss < best_loss:
                ...
                torch.save(model,str(cwd) + "/Models/" + "modflow_"+str(args.data)+"_model_" + str(itr) + ".pt")
